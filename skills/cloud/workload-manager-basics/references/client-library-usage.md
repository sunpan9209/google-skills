...
## Python: Create a General Best-Practices Evaluation

This example selects a small baseline from `OTHER` rules by tags. Adjust the
selection logic to your required categories and severity threshold.
...
parent = f"projects/{project_id}/locations/{location}"
wanted_tags = {"Security", "Reliability", "FinOps"}
...
rule_names = [
    ...
    if wanted_tags.intersection(set(rule.tags))
]
