...
## Create a General Best-Practices Evaluation
...
Replace `PROJECT_ID`, `LOCATION`, and `RULE_ID` inside the JSON before running.
List rules first and select rule IDs from the target location instead of
copying names from documentation tables.
