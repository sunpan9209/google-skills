...
posture, or custom organizational rules. In SDK automation, use `OTHER` for
this general/custom rule path and verify the available rules by listing them in
the target location.
...
Workload Manager custom rules use Rego. In SDK automation, list them through
the `OTHER` rule path with `custom_rules_bucket`.
...
5. Add custom Rego rules for internal controls that do not exist in the built-in
   catalog.
